...
def write_article(request):
    if request.method=='POST':
        form = Write_articleForm(request.POST)
        if form.is_valid():
            ...
            'desc':cd['desc'],
            'content':cd['content'],
            'is_recommend':cd['is_recommend'],
            'click_count':cd['click_count'],
            ...
            print 1
            return render(request,'success.html',{reason:'提交文章成功'})
        else:
            return render(request,'failure.html',{reason:'文章创建失败'})
    else:
